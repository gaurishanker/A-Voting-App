...
public class JdbcRealm extends AuthorizingRealm {
    ...
    protected SaltedAuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        try {
            ...
            String[] passwordAndSalt = getPasswordForUser(conn, username);

            if (passwordAndSalt == null || passwordAndSalt[0] == null || passwordAndSalt[1] == null) {
                throw new UnknownAccountException("No account found for user [" + username + "]");
            }
            ByteSource salt = new SimpleByteSource(Hex.decode(passwordAndSalt[1]));
            info = buildAuthenticationInfo(username, passwordAndSalt[0].toCharArray(), salt);
    ...
    protected SaltedAuthenticationInfo buildAuthenticationInfo(String username, char[] password, ByteSource salt) {
    ...
    private String[] getPasswordForUser(Connection conn, String username) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                passwordAndSalt = new String[2];
                passwordAndSalt[0] = rs.getString(1);
                passwordAndSalt[1] = rs.getString(2);
